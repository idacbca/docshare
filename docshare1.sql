...
CREATE TABLE IF NOT EXISTS `admin` (
  ...
  `adname` varchar(255) NOT NULL COMMENT '用户名',
...
-- 正在导出表  book.admin 的数据：1 rows
DELETE FROM `admin`;
/*!40000 ALTER TABLE `admin` DISABLE KEYS */;
INSERT INTO `admin` (`id`, `adname`, `adpassword`, `phonenumber`) VALUES
	(123456, '1611', '1611', 123456);
...
CREATE TABLE IF NOT EXISTS `goods` (
  ...
  `goodtype` varchar(1) NOT NULL COMMENT '商品类型 1电子 0实体 ',
  PRIMARY KEY (`goodsid`) USING BTREE
) ENGINE=MyISAM DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;

-- 正在导出表  book.goods 的数据：1 rows
DELETE FROM `goods`;
/*!40000 ALTER TABLE `goods` DISABLE KEYS */;
INSERT INTO `goods` (`goodsid`, `categoryid`, `categoryname`, `goodsname`, `unitprice`, `status`, `quantity`, `goodsdetail`, `goodtype`) VALUES
	(1, 1, '数学', '高数一', 1.00, 1, 3, NULL, '1');
...
-- 正在导出表  book.orders 的数据：0 rows
DELETE FROM `orders`;
/*!40000 ALTER TABLE `orders` DISABLE KEYS */;
/*!40000 ALTER TABLE `orders` ENABLE KEYS */;

-- 导出  表 book.ordersdetail 结构
...
-- 正在导出表  book.ordersdetail 的数据：0 rows
